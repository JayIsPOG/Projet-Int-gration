...
public class Player_attack : MonoBehaviour
{
    ...
    IEnumerator AttackRoutine()
    {
        ...
        Vector2 center = (Vector2)transform.position + new Vector2(dir * attackRange, 0);

        // trouve tous les ennemis par distance (pas de physics)
        ...
        foreach (var enemy in allEnemies)
        {
            ...
            if (dist < attackRange && !alreadyHit.Contains(enemy))
            {
                alreadyHit.Add(enemy);
                enemy.TakeDamage(attackDamage);
                Debug.Log("[HIT] " + enemy.gameObject.name + " HP: " + enemy.currentHealth);
            }
        }
    ...
    // debug: dessine la hitbox dans la scene view
